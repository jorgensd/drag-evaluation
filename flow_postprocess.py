...
from xii import EmbeddedMesh, Trace
...
meshes, ffs = [], []
for N in Ns:
    domain = Rectangle(Point(-L1, -Width / 2), Point(L2, Width / 2)) - Circle(
        Point(0, 0), 1, 8 * N
    )
...
for func_space in func_spaces:
    ...
    for ix_R, R in enumerate(Rs):
        ...
        if table_type == "full":
            print(
                f"N & $p_{{drag}}$ & $u_{{drag}}$ & $\\beta_{{drag}}$ &"  # noqa: F541
                + f" $\\omega_{{drag}}$ & $\\epsilon_{{diff}}$ \\\\"  # noqa: F541
            )
        else:
            print(
                f"R & $$\\omega_{{drag}}$ & $\\epsilon_{{diff}}$  & M \\\\"  # noqa: F541
            )
        for ix, N in enumerate(Ns):
            ...
            error_omega_drag = (
                100 * abs(drag_ref_values[R] - omega_drag) / drag_ref_values[R]
            )
            error_beta_drag = (
                100 * abs(drag_ref_values[R] - beta_drag) / drag_ref_values[R]
            )

            error_pressure_drag = (
                100 * abs(pressure_ref_values[R] - p_drag) / pressure_ref_values[R]
            )

            error_visc_drag = abs(
                100 * (visc_ref_values[R] - u_drag) / visc_ref_values[R]
            )

            if table_type == "full":
                print(
                    f"{N:3.0f}& {p_drag:5.3f} \scriptsize ({error_pressure_drag:1.1f}\\%) &  {u_drag:5.3f}"
                    + f"\scriptsize ({error_visc_drag:1.1f}\\%) & {beta_drag:5.3f} &"
                    + f" {omega_drag:5.3f} &  {eps_diff:1.3f} \\\\"
                )
            else:
                print(
                    f"{R:3.0f}& {omega_drag:5.3f} ({error_omega_drag:1.1f}\\%) &"
                    + f" {eps_diff:1.3f} & {N:3.0f} \\\\"
                )
        ...
        ns_v = project(
            dot((grad(u0) + grad(u0).T) * XD, n), FunctionSpace(mesh, "CG", 1)
        )
